...
export default function TestingCenterPage() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex gap-3 flex-wrap border-b border-zinc-800 pb-2">
                <TabButton
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div>
                <h2 className="text-xl font-bold text-zinc-100 mb-2 flex items-center gap-2">
                    <Shield className="text-indigo-500" size={24} /> Verify Single Details
                </h2>
            ...
            <div className="flex gap-3">
                <input
                    ...
                    className="flex-1 px-4 py-3 bg-zinc-950 border border-zinc-800 text-zinc-100 rounded-xl focus:ring-1 focus:ring-indigo-500 outline-none transition-all placeholder:text-zinc-600"
            ...
            {result && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className={`p-6 rounded-2xl border ${result.isValid ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}>
                        <div className="flex justify-between items-start">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className={`text-sm font-semibold tracking-wider uppercase mt-1 ${result.isValid ? 'text-emerald-400' : 'text-rose-400'}`}>
                                        {result.status}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="text-center">
                                <div className="text-3xl font-black text-zinc-100">{result.score}/100</div>
                                <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Quality Score</div>
                            </div>
                        </div>

                        {result.status === 'disabled' && (
                            <div className="mt-4 p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-300 text-sm font-medium flex items-center gap-3">
                                <AlertTriangle size={18} />
                                <span>Account disabled or deactivated by provider. Unsafe to send.</span>
                            </div>
                        )}
                        {result.isFullInbox && (
                            <div className="mt-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-500 text-sm font-medium flex items-center gap-3">
                                <AlertTriangle size={18} />
                    ...
                    <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-800 shadow-sm">
                        <h4 className="font-bold text-zinc-200 mb-3 flex items-center gap-2 text-sm uppercase tracking-wider">
                        ...
                        <div className="space-y-2">
                            {result.mxRecords && result.mxRecords.length > 0 ? (
                                result.mxRecords.map((mx: string, i: number) => (
                                    <div key={i} className="font-mono text-sm bg-zinc-900 border border-zinc-800 p-3 rounded-lg text-zinc-400">
                                        {mx}
                                    </div>
                                ))
                            ) : (
                                <p className="text-zinc-600 text-sm italic">No MX records returned</p>
                            )}
                        </div>
                    </div>
                </div>
...
function ResultCard({ label, value, highlight, inverse }: any) {
    ...
    if (highlight) colorClass = "text-emerald-400";
    else if (inverse && value === 'Yes') colorClass = "text-rose-400";
    else if (!highlight && !inverse) colorClass = "text-rose-400";

    return (
        <div className="bg-zinc-950 p-5 rounded-2xl border border-zinc-800 shadow-sm">
            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-1.5">{label}</p>
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-semibold text-zinc-400 mb-2">Pasted Email List (one per line)</label>
                    <textarea
                        value={emails}
                        onChange={(e) => setEmails(e.target.value)}
                        placeholder="test1@example.com&#10;test2@example.com&#10;test3@example.com"
                        className="w-full h-48 px-4 py-3 bg-zinc-950 border border-zinc-800 text-zinc-100 rounded-xl focus:ring-1 focus:ring-indigo-500 outline-none font-mono text-sm resize-none placeholder:text-zinc-700 transition-all"
                        disabled={checking}
                    />
                ...
                <div className="flex gap-3">
                    ...
                    {results.length > 0 && (
                        <button
                            ...
                            className="px-6 py-3.5 bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 text-zinc-200 rounded-xl font-bold flex items-center gap-2 transition-colors"
                        >
                            <CheckCircle size={18} className="text-emerald-400" /> Export CSV
                        </button>
                    )}
                </div>
            </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-zinc-950 p-5 rounded-xl border border-zinc-800 text-center">
                            <p className="text-3xl font-black text-zinc-100">{results.length}</p>
                            <p className="text-xs text-zinc-500 uppercase tracking-widest mt-1 font-bold">Total</p>
                        </div>
                        <div className="bg-emerald-500/10 p-5 rounded-xl border border-emerald-500/20 text-center">
                            ...
                            <p className="text-xs text-emerald-600 uppercase tracking-widest mt-1 font-bold">Valid ({results.length > 0 ? ((validCount / results.length) * 100).toFixed(0) : 0}%)</p>
                        </div>
                        <div className="bg-rose-500/10 p-5 rounded-xl border border-rose-500/20 text-center">
                            <p className="text-3xl font-black text-rose-400">{invalidCount}</p>
                            <p className="text-xs text-rose-600 uppercase tracking-widest mt-1 font-bold">Invalid ({results.length > 0 ? ((invalidCount / results.length) * 100).toFixed(0) : 0}%)</p>
                        </div>
                    </div>

                    <div className="border border-zinc-800 rounded-2xl overflow-hidden shadow-sm bg-zinc-950">
                        <div className="max-h-96 overflow-y-auto">
                            <table className="w-full text-left">
                                <thead className="bg-zinc-900 border-b border-zinc-800 sticky top-0 shadow-sm">
                                    <tr>
                                        <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Email</th>
                                        <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-center">Valid</th>
                                        <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-center">Score</th>
                                        <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Status</th>
                                        <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Reason Context</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-zinc-800">
                                    {results.map((result, i) => (
                                        <tr key={i} className="hover:bg-zinc-900/50 transition-colors">
                                            <td className="p-4 font-mono text-sm text-zinc-300">{result.email}</td>
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`font-bold text-sm ${result.score > 70 ? 'text-emerald-400' : (result.score > 40 ? 'text-amber-500' : 'text-rose-500')}`}>
                                                    {result.score || 0}
                                                </span>
                                            </td>
                                            <td className="p-4 text-sm">
                                                <span className={`px-2.5 py-1 rounded-md text-[10px] uppercase font-bold tracking-wider border ${result.status === 'valid' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                                    result.status === 'disabled' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' :
                                                        'bg-zinc-800 text-zinc-400 border-zinc-700'
                                                    }`}>
                                                    {result.status || 'unknown'}
                                                </span>
                                            </td>
                                            <td className="p-4 text-xs text-zinc-500 truncate max-w-[200px]" title={result.reason}>
                                                {result.reason || '-'}
                                            </td>
                                        </tr>
                                    ))}
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-5 bg-zinc-950 p-6 rounded-2xl border border-zinc-800">
                <div>
                    <label className="block text-sm font-semibold text-zinc-400 mb-2">Recipient Email</label>
                    <input
                        type="email"
                        value={toEmail}
                        onChange={(e) => setToEmail(e.target.value)}
                        className="w-full px-4 py-3 bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-xl focus:ring-1 focus:ring-indigo-500 outline-none transition-all placeholder:text-zinc-700"
                        placeholder="recipient@example.com"
                    />
                </div>

                <div>
                    <label className="block text-sm font-semibold text-zinc-400 mb-2">Test Subject</label>
                    <input
                        type="text"
                        value={subject}
                ...
                <div>
                    <label className="block text-sm font-semibold text-zinc-400 mb-2">Draft Body</label>
                    <textarea
                        value={body}
                        onChange={(e) => setBody(e.target.value)}
                        className="w-full h-48 px-4 py-3 bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-xl focus:ring-1 focus:ring-indigo-500 outline-none resize-none transition-all placeholder:text-zinc-700 font-mono text-sm"
...
function ManualCampaignTab() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-5 bg-zinc-950 p-6 rounded-2xl border border-zinc-800">
                <div>
                    <label className="block text-sm font-semibold text-zinc-400 mb-2">Payload Dispatch Count</label>
                    <input
                        type="number"
                        value={count}
                        ...
                        min="1"
                        max="100"
                        className="w-full px-4 py-3 bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-xl focus:ring-1 focus:ring-amber-500 outline-none transition-all placeholder:text-zinc-700"
                    />
                </div>
                ...
                {results && (
                    <div className="p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl">
                        <h3 className="font-bold text-emerald-400 mb-4 uppercase tracking-wider text-xs">Dispatch Audit Result</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-zinc-900 border border-emerald-500/20 p-4 rounded-xl text-center">
                                <p className="text-zinc-500 uppercase font-bold text-[10px] tracking-widest mb-1">Delivered</p>
                                <p className="text-3xl font-black text-emerald-400">{results.sent || 0}</p>
                            </div>
                            <div className="bg-zinc-900 border border-rose-500/20 p-4 rounded-xl text-center">
                                <p className="text-zinc-500 uppercase font-bold text-[10px] tracking-widest mb-1">Failed</p>
                                <p className="text-3xl font-black text-rose-500">{results.failed || 0}</p>
                            </div>
                        </div>
                    </div>
                )}
