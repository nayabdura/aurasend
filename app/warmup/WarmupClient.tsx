...
function getHealth(score: number, warmupEnabled: number, isConnected: number) {
    ...
    if (!warmupEnabled) return { label: 'Warmup Off', color: 'gray', bg: 'bg-white dark:bg-zinc-900/60 text-slate-600 dark:text-zinc-400', bar: 'bg-gray-400' };
...
function TemplateEditor({ accounts, initialData, onSave, onCancel }: {
    ...
    return (
        <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-lg p-6 space-y-5">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-900 dark:text-zinc-50">{initialData?.id ? '✏️ Edit Template' : '✨ New Warmup Template'}</h3>
                <button onClick={onCancel} className="p-2 hover:bg-white dark:bg-zinc-900/60 rounded-lg text-slate-500 dark:text-zinc-400"><X size={18} /></button>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {AI_WARMUP_SUGGESTIONS.map((s, i) => (
                        <button key={i} onClick={() => applyAISuggestion(s)} disabled={aiLoading}
                            className="text-xs px-3 py-1.5 bg-slate-50 dark:bg-zinc-900/50 border border-purple-300 rounded-lg hover:bg-purple-50 hover:border-purple-500 transition-colors font-medium text-purple-700 disabled:opacity-50">
                            {aiLoading ? '...' : s.name}
                        </button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-bold text-slate-600 dark:text-zinc-400 mb-1">Template Name *</label>
                    <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                        className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none" placeholder="e.g. Casual Outreach #1" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-600 dark:text-zinc-400 mb-1">Assign to Gmail Account</label>
                    <select value={form.gmail_account_id ?? ''} onChange={e => setForm(f => ({ ...f, gmail_account_id: e.target.value ? parseInt(e.target.value) : null }))}
                        className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none bg-slate-50 dark:bg-zinc-900/50">
            ...
            {/* Tabs */}
            <div className="flex gap-2 border-b border-slate-200 dark:border-zinc-800 pb-0">
                <button onClick={() => setActiveTab('edit')}
                    className={`px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${activeTab === 'edit' ? 'border-orange-500 text-orange-700' : 'border-transparent text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:text-zinc-300'}`}>
                    ✏️ Edit
                </button>
                <button onClick={() => setActiveTab('preview')}
                    className={`px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${activeTab === 'preview' ? 'border-orange-500 text-orange-700' : 'border-transparent text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:text-zinc-300'}`}>
                    👁 Preview
                </button>
            </div>
            ...
            {activeTab === 'edit' ? (
                <div className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-slate-600 dark:text-zinc-400 mb-1">Subject Line *</label>
                        <input value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                            className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none" placeholder="Email subject..." />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-600 dark:text-zinc-400 mb-1">Email Body * <span className="font-normal text-slate-400">(Use: {'{{name}}'}, {'{{company}}'}, {'{{sender_name}}'})</span></label>
                        <textarea value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))} rows={5}
                            className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none resize-none" placeholder="Email body..." />
                    </div>

                    {/* Follow-up 1 */}
                    <div className="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                        <button onClick={() => setShowFollowup1(!showFollowup1)}
                            className="w-full px-4 py-3 text-sm font-semibold text-slate-700 dark:text-zinc-300 bg-white dark:bg-zinc-900/60 hover:bg-white dark:bg-zinc-900/60 flex items-center justify-between">
                            <span>📩 Follow-up 1 (Optional)</span>
                            {showFollowup1 ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                        </button>
                        {showFollowup1 && (
                            <div className="p-4 space-y-3 bg-slate-50 dark:bg-zinc-900/50">
                                <input value={form.followup1_subject || ''} onChange={e => setForm(f => ({ ...f, followup1_subject: e.target.value }))}
                                    className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none" placeholder="Re: original subject..." />
                                <textarea value={form.followup1_body || ''} onChange={e => setForm(f => ({ ...f, followup1_body: e.target.value }))} rows={4}
                                    className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none resize-none" placeholder="Follow-up body..." />
                            </div>
                        )}
                    </div>
                        ...
                        {showFollowup2 && (
                            <div className="p-4 space-y-3 bg-slate-50 dark:bg-zinc-900/50">
                                <input value={form.followup2_subject || ''} onChange={e => setForm(f => ({ ...f, followup2_subject: e.target.value }))}
                                    className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none" placeholder="Re: Re: original subject..." />
                                <textarea value={form.followup2_body || ''} onChange={e => setForm(f => ({ ...f, followup2_body: e.target.value }))} rows={4}
                                    className="w-full px-3 py-2 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none resize-none" placeholder="Second follow-up body..." />
                            </div>
                        )}
                    </div>
                </div>
            ) : (
                <div className="border border-slate-200 dark:border-zinc-800 rounded-xl p-4 bg-white dark:bg-zinc-900/60 space-y-3">
                    <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-lg p-4 border border-slate-200 dark:border-zinc-800">
                        <p className="text-xs text-slate-500 dark:text-zinc-400 mb-1">Subject:</p>
                        <p className="font-semibold text-slate-900 dark:text-zinc-50">{form.subject || '(no subject)'}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-lg p-4 border border-slate-200 dark:border-zinc-800">
                        <p className="text-xs text-slate-500 dark:text-zinc-400 mb-2">Body:</p>
                        <div className="text-sm text-slate-800 dark:text-zinc-200 leading-relaxed" 
                             dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(previewBody || '(empty)') }} 
                        />
                    </div>
                    {form.followup1_body && (
                        <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
                            <p className="text-xs font-bold text-indigo-600 mb-1">Follow-up 1:</p>
                            <p className="text-sm font-semibold text-slate-700 dark:text-zinc-300">{form.followup1_subject}</p>
                            <p className="text-xs text-slate-600 dark:text-zinc-400 mt-1 whitespace-pre-line">{form.followup1_body}</p>
                        </div>
                    )}
                    {form.followup2_body && (
                        <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
                            <p className="text-xs font-bold text-purple-600 mb-1">Follow-up 2:</p>
                            <p className="text-sm font-semibold text-slate-700 dark:text-zinc-300">{form.followup2_subject}</p>
                            <p className="text-xs text-slate-600 dark:text-zinc-400 mt-1 whitespace-pre-line">{form.followup2_body}</p>
                        </div>
                    )}
                </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
                ...
                <button onClick={onCancel} className="px-5 py-2.5 border border-slate-200 dark:border-zinc-800 rounded-xl text-sm font-medium text-slate-600 dark:text-zinc-400 hover:bg-white dark:bg-zinc-900/60">Cancel</button>
            </div>
        </div>
    );
}
...
function SendingWindowEditor({ account, onSave }: { account: GmailAccount; onSave: (start: string, end: string) => void }) {
    ...
    if (!isEditing) {
        return (
            <div className="flex justify-between items-center text-xs text-slate-500 dark:text-zinc-400 bg-white dark:bg-zinc-900/60 px-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-800 group">
                <span className="flex items-center gap-1"><Clock size={11} /> Warmup Window</span>
                <button onClick={() => setIsEditing(true)} className="flex items-center gap-1.5 font-bold text-slate-700 dark:text-zinc-300 hover:text-indigo-600 transition-colors">
                    {start} — {end}
        ...
        <div className="bg-indigo-50/50 p-3 rounded-xl border border-indigo-100 space-y-2">
            ...
            <div className="flex gap-2">
                <div className="flex-1">
                    ...
                    <input type="time" value={start} onChange={e => setStart(e.target.value)}
                        className="w-full text-sm font-bold bg-slate-50 dark:bg-zinc-900/50 border border-indigo-200 rounded-lg px-2 py-1.5 outline-none focus:ring-2 focus:ring-indigo-400" />
                </div>
                <div className="flex-1">
            ...
            <div className="flex justify-end gap-1.5 pt-1">
                <button onClick={() => { setStart(account.warmup_send_start || '07:00'); setEnd(account.warmup_send_end || '20:00'); setIsEditing(false); }}
                    className="p-1.5 text-slate-400 hover:text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:bg-zinc-900/50 rounded-md transition-colors"><X size={14} /></button>
...
export default function WarmupClient() {
    ...
    if (loading) return (
        <div className="flex items-center justify-center py-32">
            ...
            <span className="ml-3 text-slate-600 dark:text-zinc-400 font-medium">Loading warm-up data...</span>
        </div>
    );
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-zinc-400 mt-1 text-sm max-w-xl">Per-account warmup campaigns with dedicated templates, auto-rotation, and AI-powered suggestions.</p>
                </div>
                <div className="flex gap-3 flex-wrap">
                    <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 text-center min-w-[80px]">
                        ...
                        <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium">Warming</p>
                    </div>
                    <div className="bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 text-center min-w-[80px]">
                        ...
                        <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium">Templates</p>
                    </div>
                    <div className="bg-purple-50 border border-purple-200 rounded-xl px-4 py-3 text-center min-w-[80px]">
                        ...
                        <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium">Activities</p>
                    </div>
                    {/* Diagnose + Manual Trigger buttons */}
                    <button
                        onClick={runDiagnose}
                        disabled={diagnosing}
                        className="flex items-center gap-2 px-4 py-3 bg-slate-50 dark:bg-zinc-900/50 border-2 border-indigo-300 hover:border-indigo-500 text-indigo-700 rounded-xl font-semibold text-sm transition-all disabled:opacity-60"
                    >
            ...
            {diagnoseData && (
                <div className="bg-slate-50 dark:bg-zinc-900/50 border-2 border-indigo-200 rounded-2xl shadow-md overflow-hidden">
                        ...
                        {diagnoseData.accounts.map((acct: any) => (
                            <div key={acct.email} className="p-5">
                                <div className="flex items-start gap-3 mb-3">
                                    ...
                                    <div>
                                        <p className="font-bold text-slate-900 dark:text-zinc-50">{acct.name || acct.email}</p>
                                        {acct.name && <p className="text-xs text-slate-400">{acct.email}</p>}
                                    </div>
                                    <div className="ml-auto flex gap-4 text-xs text-slate-500 dark:text-zinc-400">
                                        <span>Day <strong className="text-slate-800 dark:text-zinc-200">{acct.warmup_day}</strong></span>
                                ...
                                {acct.last_warmup_email && (
                                    <p className="text-xs text-slate-400 mt-2 ml-6">Last sent: <span className="text-slate-600 dark:text-zinc-400 font-medium">{acct.last_warmup_email.sent_at}</span> → {acct.last_warmup_email.to_email}</p>
                                )}
                            </div>
                        ))}
                    </div>
            ...
            {/* Tab Bar */}
            <div className="flex gap-1 bg-white dark:bg-zinc-900/60 p-1 rounded-xl w-fit">
                {(['overview', 'templates', 'activity'] as Tab[]).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        className={`px-5 py-2.5 rounded-lg text-sm font-semibold capitalize transition-all ${tab === t ? 'bg-slate-50 dark:bg-zinc-900/50 text-orange-700 shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:text-zinc-300'}`}>
                ...
                <div className="space-y-6">
                    {accounts.length === 0 ? (
                        <div className="text-center py-20 bg-slate-50 dark:bg-zinc-900/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                            ...
                            <h3 className="text-xl font-bold text-slate-700 dark:text-zinc-300 mb-2">No Gmail accounts</h3>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            {accounts.map(account => {
                                ...
                                return (
                                    <div key={account.id} className={`bg-slate-50 dark:bg-zinc-900/50 rounded-2xl shadow-md border-2 transition-all hover:shadow-lg ${account.warmup_enabled ? 'border-orange-200' : 'border-slate-200 dark:border-zinc-800'}`}>
                                        {/* Card Header */}
                                        <div className={`p-5 rounded-t-2xl ${account.warmup_enabled ? 'bg-gradient-to-r from-orange-50 to-amber-50' : 'bg-white dark:bg-zinc-900/60'}`}>
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        ...
                                                        {account.name ? (
                                                            <div>
                                                                <span className="font-bold text-slate-900 dark:text-zinc-50 text-sm">{account.name}</span>
                                                                <span className="block text-xs text-slate-400 font-normal leading-tight">{account.email}</span>
                                                            </div>
                                                        ) : (
                                                            <span className="font-bold text-slate-900 dark:text-zinc-50 text-sm">{account.email}</span>
                                                        )}
                                                    </div>
                                                    <div className="flex gap-2 flex-wrap">
                                                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${health.bg}`}>{health.label}</span>
                                                        <span className="text-xs text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800 px-2 py-0.5 rounded-full capitalize">{account.auth_method}</span>
                                                ...
                                                <div className="text-right">
                                                    <p className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50">{score}%</p>
                                        ...
                                        <div className="p-5 space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="w-full bg-white dark:bg-zinc-900/60 rounded-full h-2.5">
                                            ...
                                            {account.warmup_enabled === 1 && (
                                                <div className="grid grid-cols-3 gap-2">
                                                    <div className="text-center bg-orange-50 rounded-lg p-2">
                                                        ...
                                                        <p className="text-[10px] text-slate-500 dark:text-zinc-400">Day</p>
                                                    </div>
                                                    <div className="text-center bg-indigo-50 rounded-lg p-2">
                                                        ...
                                                        <p className="text-[10px] text-slate-500 dark:text-zinc-400">Sent Today</p>
                                                    </div>
                                                    <div className="text-center bg-emerald-50 rounded-lg p-2">
                                                        <p className="font-extrabold text-emerald-700">{dailyTarget}</p>
                                                        <p className="text-[10px] text-slate-500 dark:text-zinc-400">Target</p>
                                                    </div>
                                                </div>
                                            )}

                                            {/* Template Assignment */}
                                            <div className="bg-white dark:bg-zinc-900/60 rounded-xl p-3">
                                                <div className="flex items-center justify-between mb-2">
                                                    <p className="text-xs font-bold text-slate-700 dark:text-zinc-300 flex items-center gap-1">
                                                ...
                                                {isAssigning ? (
                                                    <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-zinc-400"><Loader2 size={12} className="animate-spin" /> Assigning...</div>
                                                ) : (
                                                    <select
                                                        value={account.warmup_template_id ?? ''}
                                                        onChange={e => assignTemplate(account.id, e.target.value ? parseInt(e.target.value) : null)}
                                                        className="w-full text-xs px-2 py-1.5 border border-slate-200 dark:border-zinc-800 rounded-lg bg-slate-50 dark:bg-zinc-900/50 focus:ring-2 focus:ring-orange-300 outline-none"
                                                    >
                                                ...
                                                {assignedTemplate && (
                                                    <p className="text-[10px] text-slate-500 dark:text-zinc-400 mt-1 mb-3">
                                                        {assignedTemplate.followup1_body ? '✓ Has Follow-up 1' : ''} {assignedTemplate.followup2_body ? '✓ Has Follow-up 2' : ''}
                                                    </p>
                                                )}

                                                <div className="border-t border-slate-200 dark:border-zinc-800 mt-3 pt-3 flex items-center justify-between">
                                            ...
                                            <div className="flex justify-between text-xs text-slate-500 dark:text-zinc-400">
                                                <span className="flex items-center gap-1"><Clock size={11} /> Cold Campaign: <strong className="text-slate-700 dark:text-zinc-300 ml-1">{account.sent_today}/{account.daily_limit}</strong></span>
                                            </div>
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${account.warmup_enabled
                                                    ? 'bg-white dark:bg-zinc-900/60 hover:bg-slate-100 dark:bg-zinc-800/50 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-800'
                    ...
                    {warmingCount > 0 && (
                        <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-2xl shadow-md border border-slate-200 dark:border-zinc-800 p-6">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-zinc-50 mb-4">📅 Volume Scaling Schedule</h2>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="bg-white dark:bg-zinc-900/60 text-left">
                                            <th className="px-4 py-2.5 font-bold text-slate-700 dark:text-zinc-300 rounded-l-xl">Days</th>
                                            <th className="px-4 py-2.5 font-bold text-slate-700 dark:text-zinc-300">Volume</th>
                                            <th className="px-4 py-2.5 font-bold text-slate-700 dark:text-zinc-300">Phase</th>
                                            <th className="px-4 py-2.5 font-bold text-slate-700 dark:text-zinc-300 rounded-r-xl">Benefit</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {[
                                            ...
                                            <tr key={i} className="hover:bg-white dark:bg-zinc-900/60">
                                                ...
                                                <td className="px-4 py-2.5 text-slate-500 dark:text-zinc-400">{benefit}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
            ...
            {tab === 'templates' && (
                <div className="space-y-5">
                    {templateMode === 'list' && (
                        <>
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-zinc-50">Warm-Up Templates</h2>
                            ...
                            {templates.length === 0 ? (
                                <div className="text-center py-16 bg-slate-50 dark:bg-zinc-900/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                                    <FileText size={40} className="mx-auto text-gray-300 mb-3" />
                                    <h3 className="font-bold text-slate-700 dark:text-zinc-300 mb-1">No warmup templates yet</h3>
                                    <p className="text-slate-500 dark:text-zinc-400 text-sm mb-4">Create your first template or use AI suggestions</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {templates.map(t => {
                                        ...
                                        return (
                                            <div key={t.id} className="bg-slate-50 dark:bg-zinc-900/50 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow p-5">
                                                <div className="flex justify-between items-start mb-3">
                                                    <div>
                                                        <h3 className="font-bold text-slate-900 dark:text-zinc-50">{t.name}</h3>
                                                        <div className="flex gap-2 mt-1">
                                                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.gmail_account_id ? 'bg-indigo-100 text-indigo-700' : 'bg-white dark:bg-zinc-900/60 text-slate-600 dark:text-zinc-400'}`}>
                                                ...
                                                <p className="text-xs text-slate-600 dark:text-zinc-400 font-semibold">Subject: <span className="font-normal">{t.subject}</span></p>
                                                <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1 line-clamp-2">{t.body}</p>
                                                {usedBy.length > 0 && (
            ...
            {tab === 'activity' && (
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-zinc-50">Warm-Up Activity Log</h2>
                            <p className="text-sm text-slate-400 mt-0.5">{logs.length} events recorded across all inboxes</p>
                        </div>
                        <button onClick={loadAll} className="flex items-center gap-2 text-sm font-semibold text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:text-zinc-50 px-3 py-2 rounded-xl hover:bg-slate-100 dark:bg-zinc-800/50 border border-slate-200 dark:border-zinc-800 transition-colors">
                    ...
                    {accounts.some(a => a.warmup_enabled) && (
                        <div className="bg-white dark:bg-zinc-900/60 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-100 dark:border-zinc-800/80 flex items-center gap-2">
                                <BarChart3 size={16} className="text-orange-500" />
                                <h3 className="font-bold text-slate-900 dark:text-zinc-50 text-sm">Per-Inbox Warmup Stats</h3>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead className="bg-slate-50 dark:bg-zinc-900/50 border-b border-slate-100 dark:border-zinc-800/80">
                                        <tr>
                                            {['Inbox', 'Status', 'Day', 'Health', 'Sent Today', 'Total Warmup Sent', 'Last Sent'].map(h => (
                                                <th key={h} className="text-left px-4 py-3 font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider text-xs">{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50">
                                        {accounts.filter(a => a.warmup_enabled || a.warmup_sent_today > 0).map(a => {
                                            ...
                                            return (
                                                <tr key={a.id} className="hover:bg-slate-50 dark:bg-zinc-900/50 transition-colors">
                                                    <td className="px-4 py-3">
                                                        <div className="font-semibold text-slate-900 dark:text-zinc-50 text-sm">{a.email}</div>
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-xs font-bold text-slate-600 dark:text-zinc-400">{score}%</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 font-bold text-indigo-600">{a.warmup_sent_today || 0}</td>
                                                    <td className="px-4 py-3 text-slate-600 dark:text-zinc-400">{logs.filter(l => l.gmail_account_id === a.id).length}</td>
                                                    <td className="px-4 py-3">
                                                        {lastSentMs ? (
                                                            <div>
                                                                <div className="text-xs font-semibold text-slate-700 dark:text-zinc-300">{timeAgoStr}</div>
                    ...
                    {logs.length === 0 ? (
                        <div className="text-center py-16 bg-slate-50 dark:bg-zinc-900/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                            ...
                            <h3 className="font-bold text-slate-700 dark:text-zinc-300">No warmup activity yet</h3>
                        ...
                        <div className="bg-white dark:bg-zinc-900/60 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-100 dark:border-zinc-800/80 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-slate-900 dark:text-zinc-50 text-sm">Recent Events</h3>
                                </div>
                            ...
                            <div className="divide-y divide-slate-50">
                                {logs.map(log => {
                                    ...
                                    return (
                                        <div key={log.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50 dark:bg-zinc-900/50 transition-colors">
                                            <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg whitespace-nowrap border ${typeColors[log.type] || 'bg-slate-100 dark:bg-zinc-800/50 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-800'}`}>
                                                {typeLabel[log.type] || log.type}
                                            </span>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-semibold text-slate-900 dark:text-zinc-50 truncate">{log.subject || '(no subject)'}</p>
                                                <div className="flex items-center gap-3 text-xs text-slate-400 mt-0.5">
                                                    <span className="font-medium text-slate-600 dark:text-zinc-400">{log.account_email || log.gmail_email}</span>
                                                    {log.to_email && <span>→ {log.to_email}</span>}
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <div className="text-xs font-semibold text-slate-500 dark:text-zinc-400">{ago}</div>
...
function ContactsModal({ accountId, onClose }: { accountId: number; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-2xl shadow-xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900/60">
                    <h3 className="font-bold text-slate-900 dark:text-zinc-50 flex items-center gap-2"><Users size={18} className="text-indigo-500" /> Dedicated Warmup Contacts</h3>
                    <button onClick={onClose} className="p-1.5 hover:bg-slate-100 dark:bg-zinc-800/50 rounded-lg text-slate-500 dark:text-zinc-400"><X size={16} /></button>
                </div>

                <div className="p-5 overflow-y-auto flex-1 space-y-5">
                    <div>
                        <p className="text-xs font-semibold text-slate-600 dark:text-zinc-400 mb-2">Bulk Add List (CSV style)</p>
                        <textarea value={bulkInput} onChange={e => setBulkInput(e.target.value)}
                            placeholder="email@example.com, John Doe&#10;test@domain.com, Jane Smith"
                            rows={3}
                            className="w-full text-sm border border-slate-200 dark:border-zinc-800 rounded-lg p-3 resize-none focus:ring-2 focus:ring-indigo-400 outline-none"
                        />
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <p className="text-xs font-semibold text-slate-600 dark:text-zinc-400">Current Warmup Targets ({contacts.length})</p>
                            {contacts.length > 0 && <span className="text-[10px] text-slate-400">Sent / Replied</span>}
                        </div>
                        {loading ? <p className="text-xs text-slate-500 dark:text-zinc-400">Loading...</p> : contacts.length === 0 ? (
                            <div className="text-center p-5 bg-white dark:bg-zinc-900/60 rounded-xl border border-slate-200 dark:border-zinc-800">
                                <p className="text-sm text-slate-500 dark:text-zinc-400 font-medium">No dedicated contacts.</p>
                            ...
                            <div className="space-y-2">
                                {contacts.map(c => (
                                    <div key={c.id} className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900/60 rounded-lg border border-slate-200 dark:border-zinc-800">
                                        <div>
                                            <p className="text-sm font-bold text-slate-900 dark:text-zinc-50">{c.email}</p>
                                            <p className="text-xs text-slate-500 dark:text-zinc-400">{c.name || 'No name'}</p>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <div className="text-right">
                                                <p className="text-xs font-bold text-slate-700 dark:text-zinc-300">{c.sent_count} / {c.reply_count}</p>
