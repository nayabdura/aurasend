            ...
            <main className="pt-32 pb-24 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <div className="text-center max-w-4xl mx-auto mb-20">
                        ...
                        <p className="text-xl text-slate-500 leading-relaxed font-medium">Connect your inboxes effortlessly today, route data via webhooks, and stay tuned for our upcoming native CRM connections to keep your sales workflow unified.</p>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
                        {INTEGRATIONS.map((int, i) => (
                            <div key={i} className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 flex flex-col group relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between mb-8 relative z-10">
                                    ...
                                    <span className={`px-4 py-1.5 text-xs font-black uppercase tracking-widest rounded-full border ${int.status === 'Native' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                                        {int.status}
                                    </span>
                                </div>
                                <h3 className="text-2xl font-black text-slate-900 mb-3 relative z-10">{int.name}</h3>
                                <p className="text-base text-slate-500 font-medium leading-relaxed mb-6 flex-1 relative z-10">{int.desc}</p>
                    ...
                    <div className="max-w-5xl mx-auto">
                        <div className="bg-slate-950 rounded-[3rem] p-16 md:p-24 text-center text-white relative overflow-hidden shadow-2xl">
                            ...
                            <Link href="/login" className="inline-flex items-center justify-center gap-2 h-16 px-10 bg-white hover:bg-slate-100 text-slate-900 rounded-full font-bold text-lg transition-all shadow-xl hover:scale-105 relative z-10 w-full sm:w-auto">
